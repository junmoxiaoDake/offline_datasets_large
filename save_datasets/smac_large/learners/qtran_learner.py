...
from envs.one_step_matrix_game import print_matrix_status_distributional
...
class QLearner:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        if self.args.mixer == "qtran_base":
            ...
            joint_qs, vs = self.mixer(batch[:, :-1], mac_hidden_states[:,:-1]) #joint_qs就是Q_jt, vs就是V_jt
            ...
            if not self.args.double_q: # Already computed if we're doing double Q-Learning
                max_actions_current_ = th.zeros(size=(batch.batch_size, batch.max_seq_length, self.args.n_agents, self.args.n_actions), device=batch.device )
                max_actions_current_onehot = max_actions_current_.scatter(3, max_actions_current[:, :], 1)
            #max_actions_current 就是\hat{u}，不过是approximation，因为这个时候的\hat{u}不一定是真正的最佳值
            ...
            opt_error = max_actions_qvals[:,:-1].sum(dim=2).reshape(-1, 1) - max_joint_qs.detach() + vs #max_action_qvals是论文里面的Q_jt^'(\hat{u})
            ...
            #这里实现我觉得有一些问题，为了省事，只是将当前的sample出来的u当做非opt的\hat{u}来处理了。我的理解是应该判断下，当前的u到底是不是\hat{u}再处理
            #另外我觉得一个地方要注意的是其实完全可以多sample一些非最优动作（不过这个时候，涉及到如何采样非最优动作的问题，其实挺难的）
        ...
        if t_env - self.log_stats_t >= self.args.learner_log_interval:
            ...
            if self.args.env.find("one_step_matrix_game")>=0:
                ...
                # else:
                print_matrix_status(t_env, self.logger, batch, self.mixer, mac_out, mac_hidden_states)
