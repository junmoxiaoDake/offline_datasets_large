...
"""
QPlex的代码基本和qmix的q_learner是一样的，代码相对比较简单。max_q_learner(wqmix)的代码比Qplex和qmix更加复杂点。
"""
class DMAQ_qattenLearner:
    ...
    def sub_train(self, batch: EpisodeBatch, t_env: int, episode_num: int, mac, mixer, optimiser, params,
        ...
        if self.args.double_q:
            ...
            cur_max_actions = mac_out_detach.max(dim=3, keepdim=True)[1] #标准的强化学习的操作， 通过online network找max operation，但是取值是用用的target的
            target_chosen_qvals = th.gather(target_mac_out, 3, cur_max_actions).squeeze(3)
            target_max_qvals = target_mac_out.max(dim=3)[0]

            # print(cur_max_actions.squeeze(3).shape) #torch.Size([128, 2, 2])
            # print(cur_max_actions.squeeze(3).shape + (self.n_actions,)) #torch.Size([128, 2, 2, 3])
            ...
            #target.scatter(dim, index, source) 下面一句话的意思是将1映射到cur_max_actions_onehot的第3个dim中去，按照的cur_max_actions这个index
            #其中cur_max_actions的维度为batch_size, n_seq, n_agents, 1 二cur_max_actions_onehot的维度为batch_size, n_seq, n_agents, n_commands
        ...
        # print(td_error.shape) #torch.Size([128, 1, 1])
        # print(mask.shape) #torch.Size([128, 1, 1])
        ...
        if t_env - self.log_stats_t >= self.args.learner_log_interval:
            ...
            if self.args.env == "one_step_matrix_game":
                print_matrix_status(t_env, self.logger, batch, self.mixer, mac_out, None, max_action_qvals)
