...
class MAXQLearner:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        if self.args.hysteretic_qmix: # OW-QMIX
            ...
            #cur_max_actions [n_batch, n_time, n_agents, 1]
            is_var = actions == cur_max_actions[:,:-1] #这个的shape是[n_batch, n_time-1, n_agents, 1]
            t2 = is_var.min(dim=2) #t2是一个tuple, (values, indicse） value 是[n_batch, n_time-1, 1]
            t3 = t2[0]
        ...
        if t_env - self.log_stats_t >= self.args.learner_log_interval:
            ...
            if self.args.env.find("one_step_matrix_game")>=0:
                ...
                print_matrix_status(t_env, self.logger, batch, self.mixer, mac_out, hidden=None, max_q_i=None, is_wqmix=True, wqmix_central_mixer=self.central_mixer, rest_mixer=None, central_mac_out=central_mac_out, rest_mac_out=None)
