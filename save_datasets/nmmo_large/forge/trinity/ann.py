...
class Env(nn.Module):
   def __init__(self, config):
      ...
      self.conv = torch.nn.Linear(648, h)
...
class MoveNet(nn.Module):
   ...
   # def forward(self, s_conv, s_flat, s_ents):
   #    stim = self.envNet(s_conv, s_flat, s_ents)
   #    arg, argIdx = self.moveNet(stim)
   #    return argIdx, (arg, argIdx)
...
class AttackNet(nn.Module):
   ...
   # def forward(self, s_conv, s_flat, s_ents, targets):
   #    stim = self.envNet(s_conv, s_flat, s_ents)
   #    atn, atnIdx = self.styleNet(stim)
   #
   #    #Embed targets
   ...
   #
   #    arg, argIdx = self.targNet(stim, vals)
   #
   #    attkOuts = ((atn, atnIdx), (arg, argIdx))
   #    return argIdx, attkOuts
...
class ANN(nn.Module):
   ...
   #    val  = self.valNet(s_conv, s_flat, s_ents)
   #
   #    # actions = ActionTree(env, ent, ActionV2).actions()
   #    # _, move, attk = actions
   #
   #    #Actions
   #    moveActionIdx, moveOuts = self.moveNet(s_conv, s_flat, s_ents)
   #    attackActionIdx, attkOuts = self.attackNet(s_conv, s_flat, s_ents, targets)
   #
   #    # action    = (move, attk)
   #    # arguments = (moveArg, attkArg)
   #    outs      = (moveOuts, *attkOuts)
   #
   #    return moveActionIdx, attackActionIdx, outs, val
