...
#替换掉上方类VecEnvServer中的ray.remote，便于调试及理解代码
class LocalVecServer:
   def __init__(self, config, args):

      self.envs_local = [core.LocalVecRealm(config, args, i)
            for i in range(args.nRealm)]
      self.step1 = 0
      self.step2 = 0
   ...
   #will result in undefined behavior. Don't do it after setup.
   def reset(self):
      env = [e.reset() for e in self.envs_local]
      return env

   def step(self, actions):
      step1StartTime = time.time()
      recvs = [e.step(pickle.dumps(a)) for e, a in
         zip(self.envs_local, actions)]
      step1EndTime = time.time()
      self.step1 += (step1EndTime - step1StartTime)
      step2StartTime = time.time()
      recvs = [pickle.loads(e) for e in recvs]
      next_result = zip(*recvs)
      step2EndTime = time.time()
      self.step2 += (step2EndTime - step2StartTime)
      print("***step1:", self.step1, "step2:", self.step2)
...
class VecEnv(Blacksmith):
   def __init__(self, config, args, renderStep):
      ...
      # self.env = LocalVecServer(config, args)
